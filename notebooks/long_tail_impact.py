...
@app.cell
def _(mo):
    mo.md(r"""# Calculate item weights""")
    return
...
def _(np, ratings):
    item_weights = np.log(
        ratings.shape[0] / np.count_nonzero(~np.isnan(ratings), axis=1)
    )
...
@app.cell
def _(calc_pearson_sim, item_weights, mean_ratings, ratings):
